...
export default class TaskCalendar {
  constructor(tasks = [], onTaskUpdate, onTaskCreate, onTaskDelete) {
    ...
    this.hourHeight = 60; // pixels por hora
      ...
      <div class="calendar-header">
        ...
        <div class="calendar-view-controls">
          <div class="view-toggle">
            <button class="view-btn ${this.currentView === 'day' ? 'active' : ''}" data-view="day">Día</button>
            <button class="view-btn ${this.currentView === 'week' ? 'active' : ''}" data-view="week">Semana</button>
          </div>
        </div>
      </div>
  ...
  setupEventListeners() {
    // Navegación
    this.container.querySelector('#calendar-prev').addEventListener('click', () => this.navigatePrevious());
    this.container.querySelector('#calendar-next').addEventListener('click', () => this.navigateNext());
    ...
    this.container.addEventListener('click', (e) => this.handleGridClick(e));
    
    // Modal de tarea rápida
    this.container.querySelector('#close-quick-task').addEventListener('click', () => this.closeQuickTaskModal());
    this.container.querySelector('#cancel-quick-task').addEventListener('click', () => this.closeQuickTaskModal());
    this.container.querySelector('#quick-task-form').addEventListener('submit', (e) => this.handleQuickTaskSubmit(e));
    
    // Cerrar modal al hacer click fuera
    this.container.querySelector('#quick-task-modal').addEventListener('click', (e) => {
  ...
  generateCalendarGrid() {
    ...
    grid += '<div class="calendar-days-header">';
  ...
  render() {
    ...
    this.container.querySelector('#calendar-current-date').textContent = this.formatCurrentDate();
    
    // Regenerar grid
    this.container.querySelector('#calendar-grid').innerHTML = this.generateCalendarGrid();
  ...
  renderTask(task) {
    ...
    const cells = this.container.querySelectorAll(`[data-date="${dateStr}"]`);
    ...
    if (task.time) {
      ...
      const targetCell = this.container.querySelector(`[data-date="${dateStr}"][data-hour="${hours}"]`);
      
      if (targetCell) {
        const taskElement = this.createTaskElement(task, true);
        taskElement.style.top = `${(minutes / 60) * this.hourHeight}px`;
  ...
  createTaskElement(task, hasTime = false) {
    ...
    taskDiv.className = `calendar-task ${task.status.toLowerCase().replace(' ', '-')}`;
    taskDiv.dataset.taskId = task._id;
    
    const endTime = hasTime && task.time ? this.calculateEndTime(task.time) : '';
    
    taskDiv.innerHTML = `
      <div class="task-content">
        <div class="task-title">${task.title}</div>
        ${task.time ? `<div class="task-time">${this.formatTime(task.time)}${endTime ? ` - ${endTime}` : ''}</div>` : ''}
        ${task.detail ? `<div class="task-detail">${task.detail}</div>` : ''}
      </div>
      <div class="task-status-indicator status-${task.status.toLowerCase().replace(' ', '-')}"></div>
    `;
  ...
  handleTaskClick(task) {
    // Aquí podrías abrir un modal de edición o mostrar detalles
    const shouldEdit = confirm(`¿Deseas editar la tarea "${task.title}"?`);
    if (shouldEdit && this.onTaskUpdate) {
      this.onTaskUpdate(task);
    }
  }

  openQuickTaskModal(date, hour) {
    ...
    if (hour) {
      this.container.querySelector('#quick-task-time').value = `${hour.padStart(2, '0')}:00`;
    }
